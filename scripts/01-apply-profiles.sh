...
# Provision Custom Volumes (Dependency for the sglang profile)
...
if ! lxc profile show sglang >/dev/null 2>&1; then
    echo "   Creating empty 'sglang' profile..."
    lxc profile create sglang
fi
cat configs/lxd/profiles/sglang.yaml | lxc profile edit sglang
